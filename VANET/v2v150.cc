...
main (int argc, char *argv[])
{
 ...
 // bool tracing = true;
  ...
  std::string traceFile = "scratch/mob150.ns_movements";
  uint32_t maxBytes = 0;
...
//1
  ...
  Ptr<Node> appSource1 = NodeList::GetNode (30);
  Ptr<Node> appSink1 = NodeList::GetNode (39);
  ...
  ApplicationContainer apps1 = source1.Install (vanetnodes.Get  (30));
  ...
  apps1 = sink1.Install (vanetnodes.Get (39));
...
AnimationInterface anim("v2v150.xml");
...
// Enabling the XML Flow Monitor
 if (enableFlowMonitor)
    {
      flowmonHelper.SerializeToXmlFile ("v2v150.flowmon", false, false);
    }
  Simulator::Destroy ();
  return 0;
}

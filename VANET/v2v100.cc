...
main (int argc, char *argv[])
{
 ...
 // bool tracing = true;
  ...
  std::string traceFile = "scratch/mob100.ns_movements";
  uint32_t maxBytes = 0;
...
//1
  ...
  Ptr<Node> appSource1 = NodeList::GetNode (28);
  Ptr<Node> appSink1 = NodeList::GetNode (82);
  ...
  ApplicationContainer apps1 = source1.Install (vanetnodes.Get  (28));
  ...
  apps1 = sink1.Install (vanetnodes.Get (82));
...
AnimationInterface anim("v2v100.xml");
...
// Enabling the XML Flow Monitor
 if (enableFlowMonitor)
    {
      flowmonHelper.SerializeToXmlFile ("v2v100.flowmon", false, false);
    }
  Simulator::Destroy ();
  return 0;
}

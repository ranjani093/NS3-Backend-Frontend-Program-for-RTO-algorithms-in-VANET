...
main (int argc, char *argv[])
{
 ...
 // bool tracing = true;
  ...
  std::string traceFile = "scratch/mob50.ns_movements";
  uint32_t maxBytes = 0;
...
//1
  ...
  Ptr<Node> appSource1 = NodeList::GetNode (68);
  Ptr<Node> appSink1 = NodeList::GetNode (51);
  ...
  ApplicationContainer apps1 = source1.Install (vanetnodes.Get  (68));
  ...
  apps1 = sink1.Install (vanetnodes.Get (51));
...
AnimationInterface anim("v2v50.xml");
...
// Enabling the XML Flow Monitor
 if (enableFlowMonitor)
    {
      flowmonHelper.SerializeToXmlFile ("v2v50.flowmon", false, false);
    }
  Simulator::Destroy ();
  return 0;
}

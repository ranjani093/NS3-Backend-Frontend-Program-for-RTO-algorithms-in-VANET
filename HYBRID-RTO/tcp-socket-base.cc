...
TcpSocketBase::SendEmptyPacket(uint8_t flags)
{
    ...
    // RFC 6298, clause 2.4
    m_rto = Max(m_rtt->GetEstimate() + Max(m_clockGranularity, m_rtt->GetVariation()*4+m_rtt->GetEstimate()), m_minRto);
...
TcpSocketBase::EstimateRtt(const TcpHeader& tcpHeader)
{
    ...
    if (!m.IsZero())
    {
        ...
        // RFC 6298, clause 2.4
        m_rto = Max(m_rtt->GetEstimate() + Max(m_clockGranularity, m_rtt->GetVariation() * 4+(m/2)),
                    m_minRto);
...
TcpSocketBase::NewAck(const SequenceNumber32& ack, bool resetRTO)
{
    ...
    { // Set RTO unless the ACK is received in SYN_RCVD state
        ...
        // RFC 6298, clause 2.4
        m_rto = Max(m_rtt->GetEstimate() + Max(m_clockGranularity, m_rtt->GetVariation()*4+(m_rtt->GetEstimate()/2)),
                    m_minRto);
